...
export const qiniuOSSUpload = async ({
  ...
  try {
    ...
    const result = await axios.get(`${QINIUOSS_IMAGE_DEFAULT_HOST}/qiniu/${config.bucket}/${config.accessKey}/${config.secretKey}`);
...
export const giteeUpload = ({
  ...
  base64Reader.onload = (e) => {
    ...
    formData.append("message", "markdown-lite upload picture");
...
export const githubUpload = ({
  ...
  base64Reader.onload = (e) => {
    ...
    const data = {
      content: base64,
      message: "markdown-lite upload picture",
    };
