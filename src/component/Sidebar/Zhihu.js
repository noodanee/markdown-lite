...
import {solveHtml, solveZhihuMath, solveZhihuTable, copySafari} from "../../utils/converter";
...
class Zhihu extends Component {
  ...
  copyZhihu = () => {
    ...
    solveZhihuMath();
    this.html = solveHtml();
    this.html = solveZhihuTable(this.html);
  ...
  render() {
    return (
      <Tooltip placement="left" mouseEnterDelay={ENTER_DELAY} mouseLeaveDelay={LEAVE_DELAY} title="复制到知乎">
        <a id="lite-sidebar-zhihu" className="lite-btn-zhihu" onClick={this.copyZhihu}>
          <SvgIcon name="zhihu" className="lite-btn-zhihu-icon" />
        </a>
      </Tooltip>
    );
  }
